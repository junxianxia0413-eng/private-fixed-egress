...
[[ $# -eq 1 ]] || die 'Usage: bash deploy/install_controller.sh network.example.com'
domain=$1
[[ ${#domain} -le 253 && $domain =~ ^([a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z]{2,63}$ ]] || die 'A valid public DNS hostname is required.'
...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-venv caddy rsync curl
...
runuser -u pfem -- env APP_ENV=production PUBLIC_URL="https://$domain" \
    ...
    "$app_dir/.venv/bin/python" -m scripts.setup --no-env
...
sed "s/__DOMAIN__/$domain/g" "$app_dir/deploy/Caddyfile.template" > "$candidate"
...
curl --fail --silent --show-error --retry 12 --retry-delay 5 --max-time 15 "https://$domain/healthz" \
    || die 'Local Controller is running, but public HTTPS is not verified. Check DNS, ports 80/443, and Caddy logs.'
